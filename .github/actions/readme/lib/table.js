const table = (arr, cfg) => {
    ...
    cfg = Object.keys(cfg)
        .map(key => ({
            key,
            max: Math.max(cfg[key].name ? cfg[key].name.length : key.length, Math.max.apply(null, arr.map(item => (cfg[key].digits ? item[key].toFixed(cfg[key].digits).length : item[key].toString().length) + (cfg[key].postfix ? cfg[key].postfix.length : 0))))
        }))
    ...
    const header = '| ' + Object.keys(cfg).map(key => (cfg[key].name || key).padEnd(cfg[key].max)).join(' | ') + ' |'
